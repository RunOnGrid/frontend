...
const SolutionCard = () => {
  ...
  return (
    <>
        ...
        <span> Grid is designed for businesses that are looking for cost-effective cloud solutions and may not have the budget for super-scale providers. It offers the simplest path to migrate to a decentralized cloud environment</span>
        </div>
        <button style={{marginTop:'28px'}} > Start migrating </button>
        <div className='clickeables-solutions'>
            <div onClick={()=>toggle(0)} className={`${selected === 0 ? 'selectedClick' : ''}`} ></div>
            <div onClick={()=>toggle(1)} className={`${selected === 1 ? 'selectedClick' : ''}`} ></div>
        </div>
        ...
        <span> Deploy in seconds to runonflux cloud from a Git repo using buildpacks or a dockerfile</span>
        ...
        <div className='fotos-solutions'>
            ...
            <Image alt='' src='/javascriptLogo.png' width={100} height={70}/>
